...
```plaintext
# claude
/init WebアプリでTODO管理をしたいです。コードは書かずに CLAUDE.md だけ準備して。
```
...
`/init` が完了すると、プロジェクトルート（`todos/` 直下）に `CLAUDE.md` が生成されます。

**新しいターミナルを開いて**、`todos` フォルダで以下を実行してください。
...
VS Code でプロジェクトが開き（すでに開いている場合は最前面に表示され）、エクスプローラーに `CLAUDE.md` が新しく表示されているはずです。クリックして内容を確認してみましょう。
