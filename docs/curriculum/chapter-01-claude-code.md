...
このチャプターでは、Claude Code に以下の 4 つを設定します。
...
> **重要:** このセクションは「書き足していくもの」です。プロジェクトを進めるなかで気づいたルール・注意点・失敗から学んだことを、必ずここに記録してください。CLAUDE.md は最初から完璧である必要はありません。使いながら育てることが大切です。

<!-- 以下にプロジェクト固有のルールや気づきを追記していく -->
- （日付）例: ○○の処理は必ず△△ファイルにまとめること
- （日付）例: ××のエラーは□□が原因だった
```

> **必須ステップ:** CLAUDE.md を書いたら終わりではありません。チャプター 2 以降の作業を進めるたびに、気づいたことや追加したいルールを「随時追記」セクションに書き足してください。これが Claude Code を「育てる」コツです。
...
- [ ] CLAUDE.md の「随時追記（必須）」セクションに、今日気づいたことを最低 1 件書き込んでいる
...
> **Figma（フィグマ）とは?** デザイナーが画面のデザインを作るためのツールです。このカリキュラムでは Chapter 3 で Figma と Claude Code を連携させて、「デザインを指定するだけでコードを自動生成する」体験をします。
...
- [ ] **CLAUDE.md 随時追記**: 「随時追記（必須）」セクションに最低 1 件の記録がある
...
このチャプターでは、Claude Code を「使えるツール」から「チームとして働ける環境」に整えました。
...
Chapter 1 で作った設定は、次の Chapter 2 以降で活きてきます。CLAUDE.md の Git ルールがブランチ管理を自動化し、エージェントが専門的な役割でサポートし、Slack Hook が作業完了を教えてくれます。
