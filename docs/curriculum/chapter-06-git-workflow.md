---
title: "Chapter 6: Issue とワークツリー"
parent: カリキュラム
nav_order: 6
---

# Chapter 6: Issue とワークツリー — 修正・改善を習慣にする

**所要時間**: 約 1.5 時間
**ゴール**: Issue を起点に、ワークツリーで安全に修正して PR を出すサイクルを体験する
**学ぶ Claude Code 機能**: `gh issue create`、`claude -w`（ワークツリー起動）、PR 作成

---

## このチャプターで学ぶこと

- GitHub Issues でやること（修正・改善）を管理する方法
- Claude Code のワークツリー機能を使って、作業場所を安全に分離する方法
- ワークツリー内で修正してコミットし、PR を作成してマージする流れ
- このサイクルを毎週の習慣として続けるためのコツ

---

このチャプターで学ぶのは「1 回きりの修正」ではなく、**毎週続けられる習慣**です。

現実のプロジェクトでは、アプリが動き始めた後も「ここを直したい」「これを改善したい」というアイデアが次々と出てきます。それを思いつくたびに直すのではなく、「Issue（イシュー）に記録してから、安全な場所で直してから取り込む」という一定のサイクルを回すことが大切です。

> **Issue（イシュー）って何？** GitHub に備わっている「やること・気になること・バグ」を記録する付箋のようなものです。タスク管理ツールの「チケット」に近い概念です。Issue に番号が自動でつくため、どの修正がどのタスクに対応しているかを後からたどれます。

> **ワークツリーって何？** 「別の机を用意して、そこだけで作業する」仕組みです。`claude -w` というオプションをつけて Claude Code を起動すると、新しい机（ワークツリー）が自動で用意され、その机の上で Claude Code が動き始めます。本体（main ブランチ）の机には一切触れず、完成したら本体に取り込みます。

**4 つのステップの流れ:**

```
Step 1: Claude Code で Issue を立てる             （15分）
Step 2: ワークツリーを作って作業場所を分離する      （15分）
Step 3: ワークツリー内で修正してコミットする        （40分）
Step 4: PR を作ってレビューしてマージする          （30分）
```

間違えても大丈夫です。途中でわからなくなったらすぐメンターに声をかけてください。

---

## 前提確認

このチャプターを始める前に、以下の状態になっているか確認してください。

- [ ] Chapter 5 が完了している
- [ ] アプリがブラウザで動作している
- [ ] GitHub CLI（`gh`）が使える状態になっている

```bash
# GitHub CLI のログイン状態を確認する
gh auth status
```

期待される出力例（ログイン済みの場合）:

```
github.com
  ✓ Logged in to github.com as yourname (keyring)
  ✓ Git operations for github.com configured to use https protocol.
```

> **`gh` コマンドとは？** GitHub を操作するコマンドラインツール（CLI）です。ブラウザを開かなくても Issue の作成・PR の作成・マージなどが全部ターミナルでできます。Claude Code から自動で使われる場面も多いです。

ログインしていない場合:

```bash
gh auth login
```

指示に従ってブラウザで認証してください。わからなければメンターに声をかけてください。

---

## Step 1: Claude Code で Issue を立てる（15分）

### このステップの目的

「直したいこと」を頭の中に溜めておくのではなく、GitHub Issues に記録する習慣をつけます。Claude Code を使えば、チャット感覚で Issue を作れます。

> **体験:** Claude Code に「Issue を作って」と頼むだけで、GitHub 上に Issue が作成されます。ブラウザで GitHub を開いて入力フォームに打ち込む手間が不要です。

[screenshot: Claude Code のチャット画面に Issue 作成を指示している様子]

### Claude Code への指示

Claude Code のチャット画面に、自分が直したいことを入力してください。以下はその例です。

> GitHub に Issue を立てて。タイトルは「例: ○○のとき△△が起きる」、内容は「[修正したい内容と理由を具体的に書く]」にして

実際には、あなたのアプリで気になっている点を入れてください。例えば：

- 「エラーが起きたとき、ユーザーに何も表示されない」
- 「ボタンのテキストがわかりにくい」
- 「ローディング中に画面が固まって見える」

Claude Code は以下のようなコマンドを実行します。

```bash
gh issue create \
  --title "[修正したいこと]" \
  --body "[詳細と理由]"
```

Issue が作成されると、URL と Issue 番号が表示されます。

```
https://github.com/yourname/yourrepo/issues/1
```

[screenshot: ターミナルに Issue の URL と番号が表示されている様子]

### Issue 一覧で確認する

```bash
# Issue の一覧を確認する
gh issue list
```

期待される出力例:

```
#1  [立てた Issue のタイトル]  about now
```

#### 確認ポイント

- [ ] `gh issue list` で Issue が表示される
- [ ] Issue に番号（`#1` など）がついている
- [ ] GitHub のブラウザ画面でも Issue が確認できる（URL を開いてみる）

> **体験:** Issue 番号がついたことで、「この修正は #1 の対応だ」と後からわかるようになります。コミットメッセージや PR の説明に `#1` と書くと、GitHub が自動でリンクを張ってくれます。

---

## Step 2: ワークツリーを作って作業場所を分離する（15分）

### ワークツリーとは

ワークツリーは「本体のプロジェクトとは別の机」を用意する仕組みです。

> **たとえ話:** 大切な書類（main ブランチ）が載っている元の机とは別に、もう 1 台の机を用意します。その新しい机の上でだけ試行錯誤し、完成したら元の机に取り込みます。元の机の書類は最後まで汚れません。

Git のワークツリーも同じです。main ブランチの内容はそのまま保ちながら、別のフォルダ（別の机）で作業ブランチの開発を進められます。

### `claude -w` で新しい机を用意する

ワークツリーを作るには、**ターミナルで `claude -w` オプションをつけて Claude Code を起動します。** このオプションひとつで「新しい机の用意」と「その机での Claude Code セッション開始」が同時に行われます。

```bash
# ターミナルでこのコマンドを実行する（Claude Code の外で実行する）
claude -w fix_issue_1
```

> **`-w` オプションとは？** `--worktree` の略です。`claude -w fix_issue_1` と実行すると、`.claude/worktrees/fix_issue_1/` というフォルダが自動で作成され、その中で Claude Code が起動します。名前（`fix_issue_1` の部分）は自分でわかりやすい名前をつけてください。

[screenshot: ターミナルで `claude -w fix_issue_1` を実行して Claude Code がワークツリー内で起動している様子]

Claude Code が起動したら、現在どの作業場所にいるかを確認しましょう。

```bash
# 現在のブランチを確認する（Claude Code 内のターミナルで実行）
git branch --show-current
```

期待される出力例:

```
fix_issue_1
```

> **名前なしで起動した場合:** `claude -w` とだけ入力すると、ランダムな名前のワークツリーが自動で作られます。名前を指定したほうが後から何のための作業場所かわかりやすいため、`claude -w fix_issue_1` のように名前をつけることをおすすめします。

### ワークツリーの確認

別のターミナルを開いて、元のプロジェクトフォルダ（メインの机）からワークツリーの一覧を確認できます。

```bash
# 現在のワークツリー一覧を確認する
git worktree list
```

期待される出力例:

```
/home/yourname/myproject                                    abc1234 [main]
/home/yourname/myproject/.claude/worktrees/fix_issue_1     def5678 [fix_issue_1]
```

[screenshot: git worktree list の出力。メインと新しいワークツリーが並んでいる様子]

> **2 つの机が並んでいる:** main（元の机）と fix_issue_1（作業用の新しい机）が同時に存在しています。それぞれ独立しているため、片方での変更がもう片方に影響することはありません。

> **注意:** `.claude/worktrees/` は Claude Code が自動で管理するフォルダです。直接触ったり削除したりしないようにしてください。

#### 確認ポイント

- [ ] `claude -w fix_issue_1` を実行して Claude Code が起動した
- [ ] `git branch --show-current` で `fix_issue_1` と表示される
- [ ] `git worktree list` でワークツリーが 2 つ表示される

---

## Step 3: ワークツリー内で修正してコミットする（40分）

### ワークツリー内で作業する

`claude -w fix_issue_1` で起動した Claude Code は、最初からワークツリー（新しい机）の上で動いています。元のプロジェクトフォルダ（main ブランチの机）には一切触れません。「今どの机で作業しているか」は以下のコマンドで確認できます。

```bash
# 現在のブランチを確認する（Claude Code 内のターミナルで実行）
git branch --show-current
```

期待される出力:

```
fix_issue_1
```

> **体験:** 別のターミナルウィンドウを開いて元のプロジェクトフォルダを見ると、何も変わっていないことがわかります。ワークツリー側でどれだけファイルを編集しても、元の机の書類（main ブランチ）は安全なままです。これがワークツリーの最大の利点です。

[screenshot: 2 つのターミナルウィンドウ。左がメインフォルダ（main ブランチ）、右が `claude -w` で起動したワークツリー（fix_issue_1 ブランチ）を示している様子]

---

### 修正 1: 最初の修正をする

Issue #1 の内容に沿って修正を進めます。Claude Code に指示を出します。

> Issue #1 の修正をして。[修正してほしい内容を具体的に書く]

例えば次のように指示します：

- 「エラー時にユーザーにメッセージを表示して」
- 「入力が空のまま送信できないようにして」
- 「○○ボタンのテキストを△△に変えて」

Claude Code がコードを修正します。

[screenshot: Claude Code がコードを生成して編集している様子]

#### 動作確認

修正が終わったら、ブラウザでアプリを開いて意図した通りに動くか確認します。

確認ポイント:
- [ ] 修正した機能が期待通りに動いている
- [ ] 既存の機能が壊れていない

確認できたらコミットします。

> 今の変更をコミットして。コミットメッセージは「fix: [修正内容を日本語で書く] (#1)」にして

```bash
# Claude Code が実行するコマンド
git add .
git commit -m "fix: [修正内容を日本語で書く] (#1)"
```

> **コミットメッセージに `(#1)` を入れる理由:** GitHub が Issue #1 とこのコミットを自動でリンクしてくれます。後から「この修正はどの Issue への対応か」をすぐにたどれます。

[screenshot: コミット完了後のターミナル画面。コミットハッシュとメッセージが表示されている]

---

### 修正 2: 追加の改善をする（任意）

追加で改善タスクがある場合は、引き続き同じワークツリー内で修正します。

> [追加で修正したい内容を書く]

確認ポイント:
- [ ] 修正した機能が期待通りに動いている
- [ ] 既存の機能が壊れていない

```bash
git add .
git commit -m "fix: [追加の修正内容を日本語で書く] (#1)"
```

---

### Step 3 の確認ポイントまとめ

```bash
# コミット履歴を確認する
git log --oneline
```

期待される出力例:

```
a3f2c1d fix: [追加の修正内容] (#1)
b8e4d2a fix: [最初の修正内容] (#1)
c9f1b3e feat: [前のチャプターまでのコミット]
...
```

- [ ] `fix:` プレフィックスのコミットが 1 件以上ある
- [ ] コミットメッセージに `(#1)` が含まれている
- [ ] ブラウザでアプリが正常に動作している

> **体験:** コミットメッセージに `(#1)` を付けると、GitHub の Issue ページにこのコミットが自動的に紐付きます。「この Issue に対してどんな修正がされたか」が一目でわかる開発の記録が残ります。

[screenshot: git log --oneline の出力。fix: プレフィックスのコミットが並んでいる様子]

---

## Step 4: PR を作ってレビューしてマージする（30分）

### PR とは

PR（プルリクエスト）は「この修正を本体（main ブランチ）に取り込んでください」という申請です。

> **たとえ話:** コピーに書き込んだ内容を「元の書類に反映していいですか？」と確認するプロセスです。チームであれば誰かにレビューしてもらい、問題なければ取り込まれます。一人で作業する場合でも、「どんな変更をしたか」の記録として PR を残す習慣がおすすめです。

[screenshot: GitHub の PR 作成画面の様子]

### Claude Code への指示

> ワークツリーの変更を push して、PR を作って。タイトルは「fix: [修正内容を日本語で書く]」、Issue #1 をクローズする形にして

Claude Code は以下の操作を行います。

```bash
# ブランチをリモート（GitHub）に push する
git push -u origin fix_issue_1

# PR を作成する
gh pr create \
  --title "fix: [修正内容を日本語で書く]" \
  --body "## 変更内容

- [修正した内容を箇条書きで書く]
- [追加で修正した内容があれば書く]

Closes #1"
```

> **`Closes #1` とは？** PR の説明文に `Closes #1` と書くと、PR がマージされたとき Issue #1 が自動的に「クローズ（完了）」されます。手動で Issue を閉じる必要がなくなります。

PR が作成されると URL が表示されます。

```
https://github.com/yourname/yourrepo/pull/2
```

[screenshot: Claude Code が PR を作成した後、PR の URL がターミナルに表示されている様子]

### PR を確認する

```bash
# PR の一覧を確認する
gh pr list
```

期待される出力例:

```
#2  fix: [修正内容]  fix_issue_1  about now
```

### PR をレビューする

PR を作ったら、すぐマージするのではなく、まず「意図した通りの変更になっているか」を確認します。これがレビューです。

> **たとえ話:** コピーに書き込んだ内容を元の書類に反映する前に、「書き直した内容は正しいか、余計なことを書いていないか」を一度読み返すようなものです。一人での作業でも、この「一度立ち止まって確認する」習慣がミスを防ぎます。

> **体験:** Claude Code に「PR #2 の変更内容をレビューして」と頼むだけで、差分の確認からチェック項目の整理まで行ってくれます。コードを読めなくても「意図した箇所だけが変わっているか」を一緒に確認できます。

#### 方法 1: Claude Code にレビューを依頼する

> PR #2 の変更内容をレビューして。Issue #1 の意図通りの修正か、壊れている箇所はないか確認して

Claude Code は以下のような操作で確認します。

```bash
# PR の詳細情報を確認する
gh pr view 2

# 変更された差分を確認する
gh pr diff 2
```

> **`gh pr diff` とは？** PR に含まれる変更の「前後の差分」を表示するコマンドです。`+`（追加された行）と `-`（削除された行）で変更内容が一覧できます。Claude Code はこの差分を読んで「意図した修正だけが含まれているか」を確認します。

[screenshot: `gh pr diff` の出力。変更された行が + と - で表示されている様子]

#### 方法 2: ブラウザで PR の差分を確認する

PR の URL をブラウザで開くと、GitHub の画面上でも差分を視覚的に確認できます。

```
https://github.com/yourname/yourrepo/pull/2
```

GitHub の「Files changed」タブをクリックすると、変更されたファイルと差分が色付きで表示されます。緑が追加行、赤が削除行です。

[screenshot: GitHub の PR 画面「Files changed」タブ。差分が緑と赤で色付きに表示されている様子]

#### レビューの観点

以下の点を確認します。

- 意図した箇所だけが変更されている（関係のないファイルが変わっていない）
- 修正した機能がブラウザで正常に動作している
- 既存の機能が壊れていない

#### レビューのチェックリスト

- [ ] `gh pr diff 2` または GitHub のブラウザ画面で差分を確認した
- [ ] 変更されたファイルが Issue #1 の修正に関係するものだけである
- [ ] ブラウザでアプリを開いて、修正した機能が期待通りに動いている
- [ ] 既存の別の機能が壊れていない

#### レビューで指摘があった場合の対応

> **たとえ話:** コピーに書き込んだ内容を元の机の書類に反映する前に、確認した人から「ここは違う」と言われたとします。そのときは同じコピーに赤ペンで訂正して、もう一度確認してもらえばいい。何度も確認を繰り返してから元の書類に反映するのは、完成度を高めるための自然なプロセスです。開発でも同じです。レビュー → 修正 → 再確認のループは「失敗」ではなく「品質を守る習慣」です。

チェックリストで気になる点が見つかったり、レビューで指摘があった場合は、以下の流れで対応します。

**1. ワークツリーの Claude Code に戻る**

- まだセッションを閉じていなければ、そのままチャット画面に戻れます
- 閉じていた場合は、ターミナルで再度起動します

```bash
# 同じワークツリーで Claude Code を再度起動する
claude -w fix_issue_1
```

> **`-w` で再起動しても大丈夫:** すでに `fix_issue_1` ワークツリーが存在する場合、同じ名前で `claude -w fix_issue_1` を実行すると既存のワークツリーで Claude Code が起動します。作業内容は消えていません。

**2. 指摘内容を Claude Code に伝えて修正を依頼する**

> PR のレビューで「[指摘された内容]」を指摘されたので修正して

例えば次のように伝えます：

> PR のレビューで「エラー時にもローディング表示が残ったままになる」と指摘されたので修正して

Claude Code がコードを修正します。

[screenshot: ワークツリーの Claude Code に指摘内容を伝えて修正を依頼している様子]

**3. 修正を確認してコミットする**

- [ ] ブラウザでアプリを開き、指摘箇所が修正されていることを確認した
- [ ] 既存の機能が壊れていない

```bash
git add .
git commit -m "fix: [修正した内容] (#1)"
```

**4. push すると同じ PR に自動で追加される**

> 修正を push して

```bash
git push origin fix_issue_1
```

push すると、追加コミットが自動的に PR #2 に反映されます。GitHub のブラウザ画面を開くと、新しいコミットが PR の履歴に追加されているのが確認できます。

[screenshot: GitHub の PR 画面。追加コミットが PR の履歴に表示されている様子]

**5. 再度レビューする**

「PR をレビューする」のチェックリストに戻り、もう一度確認します。問題がなくなったらマージに進みます。

> **体験:** 「1 回で完璧に仕上げなければいけない」というプレッシャーは不要です。修正 → push → 再確認のループを気軽に繰り返せるのがワークツリーと PR の仕組みの利点です。このサイクルを回した回数だけ、アプリの品質が積み上がっていきます。

---

### PR をマージする

レビューで問題がなければ、main ブランチに取り込みます。

> PR #2 をマージして

```bash
# Claude Code が実行するコマンド
gh pr merge 2 --merge
```

> **`--merge`（マージ方式）について:** PR のマージには複数の方式があります。`--merge` は通常のマージで、コミット履歴がそのまま残ります。チームのルールに合わせて `--squash`（コミットをまとめる）や `--rebase` を使い分けることもあります。このチュートリアルでは `--merge` を使います。

マージが完了すると、`fix_issue_1` ブランチの変更が main に取り込まれます。

[screenshot: gh pr merge の実行後、マージ完了のメッセージが表示されている様子]

#### 確認ポイント

- [ ] `gh pr list` で PR が表示される
- [ ] マージが完了した
- [ ] Issue #1 が自動でクローズされている（`gh issue list` または GitHub のブラウザ画面で確認）
- [ ] `git log --oneline` で main ブランチにマージコミットが追加されている

```bash
# main ブランチに切り替えてログを確認する
git checkout main
git pull
git log --oneline
```

期待される出力例:

```
e5f1a2b Merge pull request #2 from yourname/fix_issue_1
a3f2c1d fix: [追加の修正内容] (#1)
b8e4d2a fix: [最初の修正内容] (#1)
...
```

> **体験:** Issue → ワークツリー → 修正 → PR → マージ という一連のサイクルが完了しました。GitHub の Issue ページを開くと「Closed by PR #2」と表示されているはずです。修正とタスクが自動的に紐付いています。

---

### ワークツリーを片付ける

マージが終わったワークツリーは不要になります。片付けておきましょう。

> マージ済みのワークツリーを片付けて

Claude Code は以下を実行します。

```bash
# ワークツリーを削除する
git worktree remove .claude/worktrees/fix_issue_1

# マージ済みのブランチを削除する
git branch -d fix_issue_1
```

```bash
# ワークツリーが片付いたか確認する
git worktree list
```

期待される出力:

```
/home/yourname/myproject  abc1234 [main]
```

- [ ] ワークツリーが 1 つ（main のみ）になっている

> **こまめに片付ける理由:** 使い終わったワークツリーをそのままにしておくと、どのブランチが現在進行中なのかわかりにくくなります。PR をマージしたらすぐ片付けるのが習慣のコツです。

---

## うまくいかない場合

**`gh auth status` でエラーが出る場合:**

```bash
# 再度ログインする
gh auth login
```

ブラウザで認証を求められます。「GitHub.com」→「HTTPS」→「Login with a web browser」を選択して進んでください。

**`claude -w` でワークツリーが作れない場合:**

同じ名前のワークツリーがすでに存在する場合にエラーになることがあります。

```bash
# 既存のワークツリー一覧を確認する
git worktree list

# 使われなくなった古いワークツリー情報を整理する
git worktree prune
```

それでも解決しない場合はメンターに声をかけてください。

**PR のマージ後に main に変更が反映されていない場合:**

```bash
# main ブランチを最新状態に更新する
git checkout main
git pull origin main
```

**コミットメッセージを間違えた場合:**

まだ push していない場合は修正できます。

> さっきのコミットメッセージを修正して。正しいメッセージは「fix: [正しい修正内容] (#1)」にして

Claude Code が `git commit --amend` で修正します。

---

## 毎週の習慣として定着させる

このチャプターで体験した「Issue → ワークツリー → 修正 → PR」のサイクルは、週 1 回のルーティンとして続けることでアプリが少しずつ良くなります。

### 週次サイクルのイメージ

```
月曜日: 気になることを Issue に記録する（5分）
         「ボタンの色が地味」「エラー時に画面が固まる」など

火〜木: ワークツリーで修正する（空き時間に少しずつ）

金曜日: PR を作ってマージする（15分）
```

### 毎週使えるコマンドとテンプレート

**Issue を立てるとき（既存の Claude Code セッション内で）:**

> GitHub に Issue を立てて。タイトルは「[修正したいこと]」、内容は「[詳細と理由]」にして

**ワークツリーを作って作業を始めるとき（ターミナルで実行）:**

```bash
# Issue の番号や内容がわかる名前をつけて起動する
claude -w fix_issue_[番号]
```

> このコマンドひとつで「新しい机の用意」と「その机での Claude Code 起動」が完了します。

**起動した Claude Code 内で修正を依頼するとき:**

> Issue #[番号] の修正をして。[修正内容の説明]

**PR を作るとき（ワークツリーの Claude Code 内で）:**

> ワークツリーの変更を push して PR を作って。Issue #[番号] をクローズする形にして

**片付けるとき（PR マージ後）:**

> マージ済みのワークツリーを片付けて

> **習慣化のコツ:** Issue を「大きなバグだけ立てるもの」と思う必要はありません。「ボタンの文字が読みにくい」「ローディング中に何も表示されない」といった小さな気づきもどんどん Issue に記録しましょう。記録することで「直すべきことの一覧」が育っていきます。

---

## このチャプターで学んだこと

| 機能 | 体験した内容 |
|------|-------------|
| **`gh issue create`** | Claude Code から GitHub Issues を作成し、番号付きでタスクを管理できることを体験した |
| **`claude -w`** | ターミナルで `claude -w 名前` を実行するだけで、新しい作業場所（ワークツリー）が自動で用意され Claude Code が起動することを体験した |
| **Git ワークツリー** | ワークツリーで main ブランチに影響を与えず安全に修正できることを体験した |
| **コミットと Issue の紐付け** | コミットメッセージに `(#番号)` を付けることで GitHub が自動でリンクを張ることを確認した |
| **PR レビュー** | `gh pr diff` や GitHub のブラウザ画面で変更差分を確認し、意図した修正だけが含まれているかをレビューする習慣を体験した |
| **PR 作成・マージ** | `gh pr create` と `gh pr merge` で、ブラウザを使わず PR のライフサイクル全体を完結できることを体験した |
| **`Closes #番号`** | PR のマージと同時に Issue が自動でクローズされる仕組みを確認した |
| **週次サイクル** | Issue → `claude -w` 起動 → 修正 → PR という習慣として繰り返せるサイクルを体験した |

---

## チャプター全体の確認ポイント

このチャプターの全作業が終わったら、以下をまとめて確認してください。

- [ ] `gh issue list` で少なくとも 1 件の Issue が作成されている
- [ ] `git log --oneline` で `fix:` プレフィックスのコミットが 1 件以上ある
- [ ] `gh pr diff` または GitHub の「Files changed」タブで差分を確認した
- [ ] PR がマージされている（`gh pr list --state merged`）
- [ ] Issue が「Closed」になっている（`gh issue list --state closed`）
- [ ] `git worktree list` でワークツリーが 1 つ（main のみ）になっている
- [ ] ブラウザでアプリが正常に動作している

---

## 次のチャプターへ

Issue とワークツリーを使った修正サイクルが身につきました。

次の **Chapter 7: Vercel デプロイ** では、このアプリをインターネット上に公開します。あなたが作った TODO アプリに、世界中からアクセスできる URL が割り当てられます。「自分の部屋で作った料理を、レストランでみんなに提供する」感覚を体験しましょう。
